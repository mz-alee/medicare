...
const WeeklyTimetable = () => {
  return (
    <div  className="border rounded-lg  relative border-[#417978]  lg:h-[70vh] w-full md:w-[30vw] gap-4 flex flex-col items-center capitalize py-2 justify-between  px-6">
      <div className=" absolute  flex flex-col items-end gap-3 left-2 top-2">
        <Image className=" w-8  " src={plusBg} alt="plus bg" />
        <Image className=" w-6   ml-3" src={plusBg} alt="plus bg" />
      </div>
      <h1 className="text-[#205454] border-b border-[#417978] font-[500] w-fit text-md md:text-[1.8vw]">
        weekly timetable
      </h1>
      {Array.from({ length: 6 }, (_, i) => i + 1).map((items, index) => {
        return (
          <div className="border-dashed border-b-1 pb-1  text-[12px] md:text-[1vw] border-gray-300 flex justify-between w-full ">
            <h1>monday</h1>
            <p>9:00am-4:00pm</p>
          </div>
        );
